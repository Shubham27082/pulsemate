  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await registerDoctor({
        ...
        documentUrl: splitCommaValues(payload.certificates)[0] || '',
        consultationFee: Number(payload.consultationFee),
        onlineConsultationEnabled: payload.onlineAvailable,
      });
  ...
  return (
    <StaffPortalLayout>
      <div className="mx-auto max-w-3xl">
        <div className="rounded-[2rem] border border-white/90 bg-white/92 p-6 shadow-[0_28px_80px_rgba(125,162,196,0.16)] backdrop-blur sm:p-8">
          <div className="mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <TextInput label="Phone number" type="tel" value={form.phone} onChange={(value) => updateField('phone', value)} placeholder="9876543210" />
          ...
          <div className="mt-6 border-t border-gray-100 pt-6 text-center">
            <p className="text-sm text-gray-500">
              Already applied?{' '}
              <Link to="/staff/login" className="font-semibold text-green-600 hover:text-green-700">
                Sign in
              </Link>
            </p>
          </div>
