    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          ...
          <span className="font-bold text-gray-900 text-lg">PulseMate</span>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        <div className="text-center mb-10">
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Welcome to PulseMate</h1>
          <p className="text-gray-500 mt-2 text-base">Select your role to continue to your portal</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-3xl">
          {ROLES.map((role) => (
            <button
              ...
              className={`flex items-start gap-4 p-5 rounded-2xl border-2 text-left transition-all duration-200 group ${role.color}`}
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${role.iconBg} ${role.iconColor}`}>
                {role.icon}
              </div>
              <div>
        ...
        <p className="text-gray-400 text-xs mt-10">
          © {new Date().getFullYear()} PulseMate Health Technologies
        </p>
      </main>
    </div>
  );
